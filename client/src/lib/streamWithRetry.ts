...
export interface StreamCallbacks {
  ...
  /** Content reset — emitted when the server suppresses previously streamed content and restarts */
  onContentReset?: () => void;
...
function parseSSELine(line: string, callbacks: StreamCallbacks): boolean {
  ...
  try {
    ...
    if (data.content_reset && callbacks.onContentReset) callbacks.onContentReset();
