...
- Name: app.spec.ts >> Sidebar Navigation >> sidebar has new task button
- Location: e2e/app.spec.ts:104:3
...
Locator: locator('nav[aria-label="Main navigation"]').getByRole('button', { name: /New task/ })
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for locator('nav[aria-label="Main navigation"]').getByRole('button', { name: /New task/ })
