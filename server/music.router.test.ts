...
describe("music router", () => {
  it("rejects unauthenticated access to generate", async () => {
    ...
    await expect(caller.music.generate({ prompt: "test song", genre: "pop" })).rejects.toThrow();
  });

  it("rejects unauthenticated access to list", async () => {
    const { ctx } = createUnauthContext();
    const caller = appRouter.createCaller(ctx);
    await expect(caller.music.list()).rejects.toThrow();
  });
  ...
  it("authenticated list returns data or DB error", async () => {
    ...
    const caller = appRouter.createCaller(ctx);
    try {
      const result = await caller.music.list();
