...
export const slidesRouter = router({
    ...
    exportPdf: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const slideHtml = slides.map((s, i) => `<div style="page-break-after:always;padding:60px;min-height:700px;border:1px solid #e5e7eb;margin-bottom:20px;background:white;border-radius:8px;"><div style="font-size:10px;color:#9ca3af;margin-bottom:40px;">Slide ${i + 1} of ${slides.length}</div><h2 style="font-size:28px;font-weight:700;color:#1a1a1a;margin-bottom:24px;">${esc(s.title || "")}</h2><div style="font-size:16px;color:#374151;line-height:1.8;white-space:pre-wrap;">${esc(s.content || "").replace(/\n/g, "<br>")}</div>${s.notes ? `<div style="margin-top:40px;padding-top:16px;border-top:1px solid #e5e7eb;font-size:12px;color:#6b7280;font-style:italic;">Notes: ${esc(s.notes)}</div>` : ""}</div>`).join("\n");
        const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${esc(deck.title || "Presentation")}</title><style>body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#f9fafb;padding:40px;margin:0;}@media print{body{padding:0;background:white;}div{page-break-inside:avoid;}}</style></head><body><h1 style="text-align:center;font-size:32px;margin-bottom:40px;color:#111827;">${esc(deck.title || "Presentation")}</h1>${slideHtml}</body></html>`;
