...
describe("aegis router", () => {
  it("rejects unauthenticated access to preFlight", async () => {
    const { ctx } = createUnauthContext();
    const caller = appRouter.createCaller(ctx);
    await expect(caller.aegis.preFlight({ prompt: "test", taskExternalId: "t1" })).rejects.toThrow();
  });

  it("rejects unauthenticated access to stats", async () => {
    const { ctx } = createUnauthContext();
    const caller = appRouter.createCaller(ctx);
    await expect(caller.aegis.stats()).rejects.toThrow();
  });
  ...
  it("authenticated stats returns data or DB error", async () => {
    ...
    const caller = appRouter.createCaller(ctx);
    try {
      const result = await caller.aegis.stats();
