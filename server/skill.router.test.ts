...
describe("skill router", () => {
  it("rejects unauthenticated access to list", async () => {
    ...
    await expect(caller.skill.list()).rejects.toThrow();
  });

  it("rejects unauthenticated access to install", async () => {
    const { ctx } = createUnauthContext();
    const caller = appRouter.createCaller(ctx);
    await expect(caller.skill.install({ skillId: "test-skill", name: "Test", description: "A test skill", category: "utility" })).rejects.toThrow();
  });
