...
describe("appPublish router", () => {
  it("rejects unauthenticated access to status", async () => {
    const { ctx } = createUnauthContext();
    const caller = appRouter.createCaller(ctx);
    await expect(caller.appPublish.status()).rejects.toThrow();
  });

  it("authenticated status returns data or DB error", async () => {
    const { ctx } = createAuthContext();
    const caller = appRouter.createCaller(ctx);
    try {
      const result = await caller.appPublish.status();
