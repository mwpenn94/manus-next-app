    ...
    if (userId) {
      try {
        ...
        if (roEnabled) {
          ...
          // Detect whether the latest user message is an explicit optimization request
          // vs casual feedback ("much better", "thanks", "looks good").
          // Only auto-trigger RO passes on the initial work request or explicit pass requests.
          const lastUserMsg = [...conversation].reverse().find(m => m.role === "user");
          const lastUserText = typeof lastUserMsg?.content === "string" ? lastUserMsg.content : "";
          const isExplicitPassRequest = /\b(optimize|pass|recursive|converge|improve|refine|iterate|another pass|next pass|keep optimizing|run.*pass|depth pass|landscape pass|adversarial pass|future.state pass|synthesis pass|fundamental redesign)\b/i.test(lastUserText);
          const hasSubstantiveWorkRequest = lastUserText.split(/\s+/).length > 15;
          const isFirstMessage = conversation.filter(m => m.role === "user").length <= 1;
          const hasPriorConvergence = conversation.some(m => m.role === "assistant" && typeof m.content === "string" && /report_convergence|convergence.*achieved|Pass \d+/i.test(m.content));
          // Only inject the MUST-perform instruction when:
          // 1. First message (initial work request) — always auto-trigger
          // 2. Explicit pass request — user asked for optimization
          // 3. Substantive new work request (>15 words) with no prior convergence
          // Otherwise, inject a passive instruction that only triggers on explicit request
          const shouldAutoTrigger = isFirstMessage || isExplicitPassRequest || (hasSubstantiveWorkRequest && !hasPriorConvergence);
          const roInstruction = shouldAutoTrigger
            ...
            : `\n\n## RECURSIVE OPTIMIZATION (Available)\nThe user has Recursive Optimization enabled (depth ${depth}, ${tempStrategy} temperature ${temp}).\n\nRecursive optimization passes are available but should ONLY be triggered when:\n- The user EXPLICITLY asks for optimization, a pass, refinement, or improvement\n- The user says "optimize", "run another pass", "improve this", "refine", "iterate", etc.\n\nDo NOT auto-trigger optimization passes on:\n- Casual feedback ("much better", "looks good", "thanks", "great")\n- Layout/formatting requests ("fix the PDF", "change the font")\n- Questions about the work ("what did you change?", "explain this")\n- Unrelated follow-up requests\n\nWhen triggered, use report_convergence(pass_number, pass_type, status, temperature=${temp}) to track passes.\nPass types: landscape, depth, adversarial. Converge after ${depth} consecutive clean passes.`;
