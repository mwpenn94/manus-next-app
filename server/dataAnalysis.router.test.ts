...
describe("dataAnalysis router", () => {
  it("rejects unauthenticated access to analyze", async () => {
    const { ctx } = createUnauthContext();
    const caller = appRouter.createCaller(ctx);
    await expect(caller.dataAnalysis.analyze({ taskId: "test", query: "test query" }))
      .rejects.toThrow();
  });

  it("authenticated analyze attempts processing", async () => {
    const { ctx } = createAuthContext();
    const caller = appRouter.createCaller(ctx);
    try {
      await caller.dataAnalysis.analyze({ taskId: "test", query: "test query" });
    } catch (err: any) {
