...
describe("mobileProject router", () => {
  ...
  it("rejects unauthenticated access to create", async () => {
    const { ctx } = createUnauthContext();
    const caller = appRouter.createCaller(ctx);
    await expect(caller.mobileProject.create({
      name: "test", framework: "react-native", platforms: ["ios"],
      bundleId: "com.test", displayName: "Test",
    })).rejects.toThrow();
  });
  ...
  it("authenticated list returns data or DB error", async () => {
    ...
    const caller = appRouter.createCaller(ctx);
    try {
      const result = await caller.mobileProject.list();
