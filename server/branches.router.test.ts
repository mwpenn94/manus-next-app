...
describe("branches router", () => {
  it("rejects unauthenticated access to create", async () => {
    ...
    await expect(caller.branches.create({
      parentTaskExternalId: "t1",
      branchPointMessageId: 1,
      label: "test branch",
    })).rejects.toThrow();
  });

  it("rejects unauthenticated access to children", async () => {
    const { ctx } = createUnauthContext();
    const caller = appRouter.createCaller(ctx);
    await expect(caller.branches.children({ taskExternalId: "t1" })).rejects.toThrow();
  });

  it("authenticated tree returns data or DB error", async () => {
    const { ctx } = createAuthContext();
    const caller = appRouter.createCaller(ctx);
    try {
      const result = await caller.branches.tree({ taskExternalId: "t1" });
